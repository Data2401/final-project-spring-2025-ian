...
```{r Loading in Data, echo=FALSE, message=FALSE}
library(readr)
...
# Get counts
male_count   <- male_data   %>% filter(bmi >= 30, diabetes == 0) %>% tally() %>% pull(n)
female_count <- female_data %>% filter(bmi >= 30, diabetes == 0) %>% tally() %>% pull(n)
...
ggplot(df, aes(x = group, y = count, fill = group)) +
  ...
  labs(
    title = "Suspcted 'overweight' people without diabetes",
    x = "",
