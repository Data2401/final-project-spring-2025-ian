...
A1C_vs_Glucose <- ggplot(diabetes_dataset) + 
...
diabetes_dataset %>% group_by(diabetes) %>% summarise(total = n())

DIABETES_DEFINITION_BROKEN <- diabetes_dataset %>% filter(HbA1c_level >= 6.5)

ggplot(DIABETES_DEFINITION_BROKEN, aes(x = HbA1c_level, y = blood_glucose_level, color = as.factor(diabetes))) + geom_line() 
...
Now we can graph the relationship between smoking and diabetes as separated by category.
